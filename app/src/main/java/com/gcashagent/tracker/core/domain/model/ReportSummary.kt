...
data class ReportSummary(
    ...
    /** Total charges across the transactions — this is the agent's income. */
    val totalChargeCentavos: Long = 0
) {
    ...
    /** Income earned from charges. */
    val incomeCentavos: Long get() = totalChargeCentavos

    companion object {
        val EMPTY = ReportSummary(0, 0, 0, 0)
        ...
        fun from(transactions: List<Transaction>): ReportSummary {
            ...
            for (t in transactions) {
                ...
                charges += t.chargeCentavos
            }
            return ReportSummary(cashIn, cashOut, transactions.size, charges)
        }
    }
}
